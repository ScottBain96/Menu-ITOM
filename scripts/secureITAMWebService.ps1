...
Copy-Item "$pwd/sourceXML/web.xml" -Destination $pwd
Copy-Item "$pwd/sourceXML/jboss-web.xml" -Destination $pwd


Write-Host "`nCreating XML files, will require some information:`n"
$UserNameDefined = Read-Host "Enter username which will be used for Web Service Authentication"
...
$filecontent = Get-Content -Path "$pwd\web.xml"
...
Set-Content -Path "$pwd\web.xml" -Value $filecontent
...
Write-Host "2) Username MUST BE $UserNameDefined"
...
Write-Host "6) What groups do you want... MUST BE: $UserRoleDefined"
...
Copy-Item "$pwd/web.xml" -Destination $itamWebPath
Copy-Item "$pwd/jboss-web.xml" -Destination $itamWebPath
...
#leaving this input as a variable in case I add any further validation.
$checkIfWorking=Read-Host "`nIf you want to confirm that it is working press enter, else you can close the script and verify on a browser"


Write-Host "`nTo confirm change is working:`n"
